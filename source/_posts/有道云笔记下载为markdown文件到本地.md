...
tags:
  - 教程
  - 有道云
title: 有道云笔记下载为markdown文件到本地
date: 2026-06-01
---
项目地址：[https://github.com/DeppWang/youdaonote-pull](https://github.com/DeppWang/youdaonote-pull)
# youdaonote-pull
...
- 兼容性修复：补充了 `beautifulsoup4` 的版本约束，避免在较老的 Python 环境里导入 `markdownify` 时报错
- 目录处理修复：脚本现在按项目根目录定位配置文件，不再依赖你从哪个工作目录启动
- Cookie 处理修复：`YNOTE_CSTK` 不再要求排在第一位，只要 `cookies.json` 里存在就能识别
- 路径处理修复：图片、附件和本地目录创建更稳，减少空格、嵌套目录和多级路径带来的失败
- 测试更新：统一了换行符断言，Windows 和非 Windows 环境下都能通过测试
- 导出隔离：下载出来的笔记目录 `youdaonote/` 已加入 `.gitignore`，不会被误提交到 GitHub
